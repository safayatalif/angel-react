...
const Book = ({ book }) => {
  ...
  return (
    <div className="w-full group h-[550px]">
      <div className="w-full transform rotate-0 group-hover:scale-105 group-hover:rotate-3 transition-transform duration-300">
        <img
          src={book?.image}
          className="w-full h-72 rounded-lg border-b-2 border-l border-r-2 border-t border-b-[#0084ff] border-l-[#005eb6] border-r-[#0084ff] border-t-[#005eb6] bg-white shadow-md dark:bg-[#18181B]"
          alt=""
        />
      </div>
      <div className="flex-1 text-white space-y-4">
        ...
        <p className="text-gray-300">{book?.description?.split(" ").slice(0, 20).join(" ")}</p>
